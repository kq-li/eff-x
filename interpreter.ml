...
and eval_stmt ctx stmt =
  Option.iter !stmt_count ~f:(fun n -> stmt_count := Some (n + 1));
  match stmt with
  | Stmt.Skip -> Ok ctx
  | Assign (x, _, _, e) ->
...
let eval ?(count = false) prog =
  if count then stmt_count := Some 0;
  let%map (_ : Value.t String.Map.t) = eval_stmt empty_ctx (Stmt.Seq prog) in
  Option.iter !stmt_count ~f:(printf "statements executed: %d\n");
  ()
